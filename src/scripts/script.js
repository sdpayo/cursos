...
$(document).ready(function() {
    var modal = $('<div class="image-modal"><span class="image-modal-close">&times;</span><img class="image-modal-content" id="modalImage"><div class="image-modal-caption" id="caption"></div></div>');
    $('body').append(modal);
    ...
    $(document).on('click', '.mbr-navbar__item.dropdown > .dropdown-toggle', function(e) {
        e.preventDefault();
        e.stopPropagation();
        var $dropdown = $(this).closest('.dropdown');
        var $menu = $dropdown.find('.dropdown-menu');
        var isOpen = $dropdown.hasClass('open') || $dropdown.hasClass('show');
        
        // Close other dropdowns
        $('.mbr-navbar__item.dropdown').removeClass('open show');
    ...
    $(document).on('click', function(e) {
        if (!$(e.target).closest('.mbr-navbar__item.dropdown').length) {
            $('.mbr-navbar__item.dropdown').removeClass('open show');
        }
    });
    ...
    // TP form -> send to Google Apps Script (if present on page)
    (function initTpForm() {
        var $form = $('#tpForm');
        if (!$form.length) return;

        const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz0KWlrQ79N5UT90s1UyiByyGWXrFAxlF1ndQhIcD3sPTLW6DdT1XWgI_Kp7OS-HG5rNg/exec';

        $form.on('submit', function(e) {
            e.preventDefault();

            var sheetName = $form.attr('data-sheet-name') || 'Respuestas';

            var payload = {
                sheetName: sheetName,
                nombre: $('#nombre').val(),
                email: $('#email').val(),
                carrera_corta: $('#carrera').val(),
                Espantapajaros_Girondo: $('#Espantapajaros_Girondo').val(),
                Cronicas_Ferreteras: $('#Cronicas_Ferreteras').val(),
                Ni_una_sola_palabra_de_amor: $('#Ni_una_sola_palabra_de_amor').val(),
                timestamp: new Date().toLocaleString('es-AR')
            };

            var $ok = $('#successMessage');
            ...
            $.ajax({
                url: SCRIPT_URL,
                method: 'POST',
                data: payload, // jQuery lo serializa como key=value&...
                // contentType por defecto: application/x-www-form-urlencoded; charset=UTF-8
                // Evita preflight; Apps Script leerá e.parameter
                ...
                error: function(xhr, status, error) {
                    ...
                    // Aunque el navegador bloquee la respuesta por CORS,
                    // el POST se envía y el Sheets suele guardar.
                    $ok.show();
                    $form[0].reset();
                    setTimeout(function(){ $ok.hide(); }, 5000);
                },
                complete: function() {
                    $btn.prop('disabled', false).text('Guardar Respuestas');
                }
            });
        });
    })();
